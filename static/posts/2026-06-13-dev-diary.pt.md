...
1. **Babashka (Scripts):** Script [fetch_stars.bb](https://github.com/patrickcmserrano/clj-dialects/blob/master/scripts/fetch_stars.bb) que lê o arquivo de metadados (`base.edn`), consulta a API do GitHub e atualiza a contagem de estrelas de cada dialeto.
...
No meu projeto principal de trading, o [ark-streams](https://github.com/patrickcmserrano/ark-streams), enfrentei um problema invisível e clássico de engenharia de redes: **conexões de WebSocket que travavam sem fechar ou disparar erros no Go**, deixando a interface vazia de dados de mercado (empty streams).
...
Centralizei o ciclo de vida de conexão em um helper genérico [ws_keepalive.go](https:/blob/master//github.com/Nexus-Quantitative/ark-streams/blob/master/internal/collector/ws_keepalive.go):
...
2. **Deploy via Runner:** Um runner self-hosted escuta por commits na master, executa [deploy-vps.sh](https://github.com/Nexus-Quantitative/ark-streams/blob/master/scripts/deploy-vps.sh) que limpa instâncias órfãs e recria os contêineres usando o docker-compose.
...
Finalmente, finalizei o módulo do executor algorítmico automatizado no Go. Implementei o [riskguard.go](https://github.com/Nexus-Quantitative/ark-streams/blob/master/internal/executor/riskguard.go) que atua como barreira entre o sinal recebido e o envio de ordens na Bitget:
