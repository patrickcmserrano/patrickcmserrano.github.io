...
import { c as currentLanguage, _ } from "../../../../chunks/simple-i18n.js";
...
function _page($$payload, $$props) {
  ...
  head($$payload, ($$payload2) => {
    $$payload2.title = `<title>${escape_html(getLoc(project.title, store_get($$store_subs ??= {}, "$currentLanguage", currentLanguage)))} — Patrick Campelo Serrano</title>`;
    $$payload2.out += `<meta name="description"${attr("content", getLoc(project.excerpt, store_get($$store_subs ??= {}, "$currentLanguage", currentLanguage)))}>`;
  });
  ...
  $$payload.out += `<!----> `;
  ThemeToggle($$payload);
  $$payload.out += `<!----></div></div></header> <main class="max-w-4xl mx-auto px-4 py-12"><div class="mb-8"><a href="/trabalho/" class="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors group"><span class="group-hover:-translate-x-1 transition-transform">←</span> ${escape_html(store_get($$store_subs ??= {}, "$_", _)("nav.backToProjects"))}</a></div> <header class="mb-12 pb-8 border-b border-slate-200 dark:border-slate-800"><div class="flex items-center justify-between mb-4"><span class="px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/40">${escape_html(getLoc(project.role, store_get($$store_subs ??= {}, "$currentLanguage", currentLanguage)))}</span> <span class="text-sm font-mono text-slate-500 dark:text-slate-400">${escape_html(getLoc(project.status, store_get($$store_subs ??= {}, "$currentLanguage", currentLanguage)))}</span></div> <h1 class="text-3xl md:text-5xl font-extrabold tracking-tight mb-3">${escape_html(getLoc(project.title, store_get($$store_subs ??= {}, "$currentLanguage", currentLanguage)))}</h1> <p class="text-xl text-slate-600 dark:text-slate-400 font-semibold mb-6">${escape_html(getLoc(project.subtitle, store_get($$store_subs ??= {}, "$currentLanguage", currentLanguage)))}</p> <div class="flex flex-wrap gap-2"><!--[-->`;
  ...
  $$payload.out += `<!--]--></div></header> <div class="space-y-12"><section><h2 class="text-2xl font-bold mb-4 tracking-tight border-l-4 border-indigo-500 pl-4">${escape_html(store_get($$store_subs ??= {}, "$_", _)("project.overview"))}</h2> <div class="text-lg leading-relaxed text-slate-700 dark:text-slate-300 space-y-4">${html(getLoc(project.details, store_get($$store_subs ??= {}, "$currentLanguage", currentLanguage)))}</div></section> <section class="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 shadow-sm"><h2 class="text-2xl font-bold mb-4 tracking-tight text-indigo-600 dark:text-indigo-400">${escape_html(store_get($$store_subs ??= {}, "$_", _)("project.architecture"))}</h2> <div class="text-base leading-relaxed text-slate-600 dark:text-slate-400 space-y-4">${html(getLoc(project.architecture, store_get($$store_subs ??= {}, "$currentLanguage", currentLanguage)))}</div></section> `;
